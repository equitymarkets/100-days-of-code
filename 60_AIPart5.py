#Day 60: It is important to demonstrate the different parts of the idea in 
...
ax = plt.figure("Higher Intelligence Sphere", figsize = (20, 20)).gca(projection='3d')
plt.title("Higher Intelligence = Sphere / Artificial Intelligence = Polyhedron")
print(" ")
xs = 8
ys = 8
zs = 8
# ~ arrow = ax.scatter(xs,ys,zs)
# ~ point = ax.plot3D(xs,ys,zs)
...
ax.annotate('O', xy=(4,6), xytext = (4,8), arrowprops=dict(arrowstyle="->"))
x1 = dia * np.outer(np.cos(a1), np.sin(b1))
y1 = dia * np.outer(np.sin(a1), np.sin(b1))
z1 = dia * np.outer(np.ones(np.size(a1)), np.cos(b1))
ax.plot_surface(x1,y1,z1, rstride=4, cstride=4, color='palegreen')
plt.show()
